...
#ifndef DUESOFTUART_H
#define DUESOFTUART_H
...
namespace arduino_due
{
  ...
  namespace soft_uart
  {
	...
	{ 
	  if ((_mode_==mode_codes::FULL_DUPLEX) ||
	      (_mode_==mode_codes::RX_MODE))
	...
	void enableRX()  { _ctx_.enableRX(); }

    void disableRX() { _ctx_.disableRX(); }
	...
	void enableRX(void)  { _tc_uart_.enableRX(); }

    void disableRX(void) { _tc_uart_.disableRX(); }
	...
	size_t write9bit(uint8_t data) 
	{
	  uint32_t snd = ((uint32_t) data) | 0x100;
      return write(snd);
	}
